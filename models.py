...
def squeeze_excitation(x,reduction_ratio,name):
    ...
    x = Dense(out_dim//reduction_ratio,name=f'{name}_ex0')(x)
    X = ELU(name=f'{name}_ex0_relu')(x)
...
def SE_ResNet(input_shape):
    ...
    #  f,t,c
    x = Permute((2,1,3),name='permute')(x_in)
    x = Conv2D(64,(3,3),strides=(2,2),padding='same',name='conv1',kernel_regularizer = regularizers.l2(l=c.WEIGHT_DECAY))(x)
    x = BatchNormalization(name='bn1')(x)
    ...
    x = Conv2D(512,(x.shape[1].value,1),strides=(1,1),padding='VALID',name='fc1')(x)
    x = BatchNormalization(name="bn_fc1")(x)
